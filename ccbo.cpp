...
int CCBO::ctcl_lunar_month(int y, int m, int d)
{
    ...
    if (lunar_month == ctcl_leap_month(y -1) && m == 1 && d < lunar_days)
        ...
        lunar_month = ctcl_round(ctcl_rem(lunar_month - 1, 12) + 1);
...
{
    QString QueryStr = QString("select note from cnote where y=%1 and m=%2 and d=%3").arg(y).arg(m).arg(d);
    QString data = ctcl_common_query("cnote", QueryStr);
...
{
    QString QueryStr = QString("select note from cnote where y=%1 and m=%2 and d=%3").arg(y).arg(m).arg(d);
    QString note = ctcl_common_query("cnote", QueryStr);

    if (textedit.isEmpty() && note.isEmpty())
        return 0;

    if (note.isEmpty())
    {   QString InsertStr = QString("INSERT INTO cnote(y,m,d,note) VALUES(%1,%2,%3,'%4');").arg(y).arg(m).arg(d).arg(textedit);
        ctcl_common_query("cnote",InsertStr);
    }
    else
    {
        QString UpdateStr = QString("UPDATE cnote SET note = '%4' where y=%1 and m=%2 and d=%3").arg(y).arg(m).arg(d).arg(textedit);
...
int CCBO::ctcl_solar_to_lunar(int y, int m, int d, struct CCalendar* ctc)
{
    ...
    QString QueryStr = QString("select Ji from yiji where Year=%1 and Month=%2 and Day=%3").arg(y).arg(m).arg(d);
